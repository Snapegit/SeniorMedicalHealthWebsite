...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="180px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="运动项目" prop="yundongxiangmu">
            <el-input v-model="ruleForm.yundongxiangmu" 
                placeholder="运动项目" clearable :disabled=" false  ||ro.yundongxiangmu"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="运动图片" v-if="type!='cross' || (type=='cross' && !ro.yundongtupian)" prop="yundongtupian">
            <file-upload
            tip="点击上传运动图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="运动图片" prop="yundongtupian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.yundongtupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="建议时长" prop="jianyishizhang">
            <el-input v-model="ruleForm.jianyishizhang" 
                placeholder="建议时长" clearable :disabled=" false  ||ro.jianyishizhang"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="建议频率" prop="jianyipinlv">
            <el-input v-model="ruleForm.jianyipinlv" 
                placeholder="建议频率" clearable :disabled=" false  ||ro.jianyipinlv"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`yundongjianyi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('yundongjianyi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`yundongjianyi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      yundongtupianUploadChange(fileUrls) {
          this.ruleForm.yundongtupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
